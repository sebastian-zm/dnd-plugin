...
export default async function apply_healing(params, userSettings) {
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    ...
    if (temporary) {
      ...
      let updated;
      try {
        updated = await store.patch(table, record.id, { temporary_hp: new_temporary_hp }, record.updated_at);
      } catch (err) {
        if (err.code === '42P01' || err.code === '42703') {
          return `Schema error: ${err.message}. Please call dnd5e24_run_migrations then retry.`;
        }
    ...
    let updated;
    try {
      updated = await store.patch(table, record.id, { current_hp: new_current_hp }, record.updated_at);
    } catch (err) {
      if (err.code === '42P01' || err.code === '42703') {
        return `Schema error: ${err.message}. Please call dnd5e24_run_migrations then retry.`;
      }
