...
async function apply_damage(params, userSettings2) {
  ...
  for (let attempt = 0; attempt < MAX_RETRIES2; attempt++) {
    ...
    if (new_current_hp === 0) {
      parts.push(entity_type === "character" ? "Unconscious \u2014 rolling death saves." : "Dead.");
    }
    return parts.join(" ");
  }
...
async function apply_healing(params, userSettings2) {
  ...
  return `Could not apply healing to "${entity}" after ${MAX_RETRIES3} attempts due to concurrent updates. Please retry.`;
}
...
async function give_item(params, userSettings2) {
  ...
  return `Could not update equipment for "${entity}" after ${MAX_RETRIES4} attempts due to concurrent updates. Please retry.`;
}
...
async function grant_xp(params, userSettings2) {
  ...
  for (const record of records) {
    ...
    if (patched === null) {
      results.push(`${record.name}: failed to update after ${MAX_RETRIES5} attempts`);
    }
  }
...
async function remove_condition(params, userSettings2) {
  ...
  return `Could not remove condition from "${entity}" after ${MAX_RETRIES6} attempts due to concurrent updates. Please retry.`;
}
...
async function remove_item(params, userSettings2) {
  ...
  return `Could not update equipment for "${entity}" after ${MAX_RETRIES7} attempts due to concurrent updates. Please retry.`;
}
...
async function roll_death_save(params, userSettings2) {
  ...
  return `Could not record death save for "${character}" after ${MAX_RETRIES8} attempts due to concurrent updates. Please retry.`;
}
...
async function use_resource(params, userSettings2) {
  ...
  return `Could not update resource for "${entity}" after ${MAX_RETRIES9} attempts due to concurrent updates. Please retry.`;
}
...
async function use_spell_slot(params, userSettings2) {
  ...
  return `Could not expend spell slot for "${character}" after ${MAX_RETRIES10} attempts due to concurrent updates. Please retry.`;
}
...
var tools = [
  ...
  },
  {
    ...
    description: "Applies damage to a character or NPC, automatically applying the target's resistances, immunities, and vulnerabilities when damage_type is provided. Resistance halves damage (round down); immunity negates it; vulnerability doubles it. Temporary HP absorbs damage after this multiplier is applied. HP cannot go below 0. Always pass the raw dice roll \u2014 do not pre-adjust for resistances.",
  ...
  },
  {
    ...
    description: "Deletes a game and all its associated data (characters, NPCs, memories) by slug. This is irreversible.",
    inputSchema: { "type": "object", "required": ["slug"], "properties": { "slug": { "type": "string", "description": "The slug of the game to delete." } } },
    handler: delete_game
  },
  {
  ...
  },
  {
    ...
    description: "Adds an item to a character's or NPC's equipment list.",
    inputSchema: { "type": "object", "required": ["game", "entity_type", "entity", "item"], "properties": { "game": { "type": "string", "description": "The slug of the game." }, "entity_type": { "type": "string", "enum": ["character", "npc"], "description": "Whether the target is a player character or an NPC." }, "entity": { "type": "string", "description": "The slug or UUID of the character or NPC." }, "item": { "type": "string", "description": "The item to add (e.g., 'Longsword +1', 'Potion of Healing')." } } },
    handler: give_item
  },
  {
  ...
  },
  {
    ...
    description: "Lists all memories for a game, returning id, slug, name, and full memory text. Call this at the start of every session to load the game's lore and context before narrating.",
    inputSchema: { "type": "object", "required": ["game"], "properties": { "game": { "type": "string", "description": "The slug of the game." } } },
  ...
  },
  {
    ...
    description: "Removes an item from a character's or NPC's equipment list. Removes the first exact match.",
    inputSchema: { "type": "object", "required": ["game", "entity_type", "entity", "item"], "properties": { "game": { "type": "string", "description": "The slug of the game." }, "entity_type": { "type": "string", "enum": ["character", "npc"], "description": "Whether the target is a player character or an NPC." }, "entity": { "type": "string", "description": "The slug or UUID of the character or NPC." }, "item": { "type": "string", "description": "The exact item string to remove (must match an entry in the equipment list)." } } },
    handler: remove_item
  },
  {
    name: "dnd5e24_rest",
  ...
  },
  {
    ...
    description: "Begins a combat encounter for a game. Sets the initiative order and advances to round 1. Each combatant entry requires a slug, entity type, display name, and initiative roll. The turn order is sorted from highest to lowest initiative. Call advance_turn after each combatant acts.",
    inputSchema: { "type": "object", "required": ["game", "combatants"], "properties": { "game": { "type": "string", "description": "The slug of the game." }, "combatants": { "type": "array", "description": "All combatants entering initiative, in any order. Will be sorted by initiative descending.", "minItems": 1, "items": { "type": "object", "required": ["slug", "entity_type", "name", "initiative"], "properties": { "slug": { "type": "string", "description": "The entity's slug (or UUID)." }, "entity_type": { "type": "string", "enum": ["character", "npc"], "description": "Whether this combatant is a player character or NPC." }, "name": { "type": "string", "description": "Display name used in turn announcements." }, "initiative": { "type": "integer", "description": "Initiative roll result (higher goes first)." } } } } } },
